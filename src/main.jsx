...
function getNewEntryModal(page){ return page==='tenants'?'new-tenants':page==='finance'?'record-payment':page==='reports'?'log-expense':page==='support'?'new-support':'new-camps'; }
...
function Finance({ctx}){
  ...
  const rows=filtered(ctx.data.charges,ctx.query,['tenant_name','company','room','receipt_no','charge_type']).filter(r=>tab==='rent'?r.charge_type==='Rent':tab==='wifi'?r.charge_type==='WiFi':Number(r.balance)>0).filter(r=>ctx.filters.building==='All'||r.camp_name===ctx.filters.building);
  ...
  const unpaidTotal=rows.filter(r=>Number(r.amount_paid||0)===0 && Number(r.balance||0)>0).reduce((a,r)=>a+Number(r.balance||0),0);
  const partialTotal=rows.filter(r=>normalize(r.status)==='partial').reduce((a,r)=>a+Number(r.balance||0),0);
  ...
  return <section>
    <div className="financeHead responsiveRow">
      <Title title="Rent & WiFi Collection" sub="Manage institutional revenue streams and outstanding balances for UAE labor accommodations."/>
      <div className="tabs">
    ...
    <Card title={`Payment Due Roster`} action={<><button onClick={()=>ctx.setModal({type:'building-filter'})} className="ghost"><Filter size={16}/> Filter By Building</button><button className="primary" onClick={()=>csvDownload('bulk_receipts.csv', rows)}>Bulk Receipt Action</button></>}><FinanceTable rows={rows} ctx={ctx}/></Card>
...
function PaymentForm({ctx, close, charge}){ return <Modal title="Record Payment" onClose={close} wide><form className="formGrid" onSubmit={async e=>{e.preventDefault(); const fd=new FormData(e.currentTarget); const amount=Number(fd.get('amount')); const chargeId=fd.get('charge_id'); await ctx.insert('payments',{ charge_id:chargeId||null, tenant_name:fd.get('tenant_name'), amount, payment_date:fd.get('payment_date'), method:fd.get('method'), receipt_no:fd.get('receipt_no'), notes:fd.get('notes') }); if(chargeId){ const ch=ctx.data.charges.find(c=>String(c.id)===String(chargeId)); if(ch){ const paid=Number(ch.amount_paid||0)+amount; await ctx.update('charges', ch.id, { amount_paid: paid, status: paid>=Number(ch.amount_due||0)?'Fully Paid':'Partial', receipt_no: fd.get('receipt_no') }); }}}}><Field label="Tenant"><input name="tenant_name" defaultValue={charge?.tenant_name} required/></Field><Field label="Charge"><select name="charge_id" defaultValue={charge?.id||''}><option value="">No linked charge / Other</option>{ctx.data.charges.map(c=><option key={c.id} value={c.id}>{c.tenant_name} - {c.charge_type} - Balance {money(c.balance)}</option>)}</select></Field><Field label="Charge Type"><select name="charge_type"><option>Rent</option><option>WiFi</option><option>Other</option></select></Field><Field label="Payment Date"><input name="payment_date" type="date" defaultValue={today()}/></Field><Field label="Payment Method"><select name="method"><option>Cash</option><option>Bank Transfer</option><option>Cheque</option><option>Card</option></select></Field><Field label="Amount Paid"><input name="amount" type="number" step="0.01" defaultValue={charge?.balance||''} required/></Field><Field label="Receipt No."><input name="receipt_no" defaultValue={`LC-${Date.now().toString().slice(-6)}`}/></Field><Field label="Notes / Other Charges"><textarea name="notes" placeholder="Optional rent adjustment, other charge, reference..."/></Field><button className="primary">Save Payment</button></form></Modal> }
